...
public class ZoneTrigger : MonoBehaviourPun
{
    ...
    private string lastTeam = "neutral";   // <- αποθηκεύουμε τη τελευταία ενεργή ομάδα
    ...
    private void Update()
    {
        ...
        // ποια ομάδα βρίσκεται (ή neutral)
        string currentTeam = "neutral";
        ...
        // progress μόνο όταν υπάρχει μία ομάδα
        if (currentTeam == "blue")
        {
        ...
        // αν άλλαξε ομάδα -> στέλνουμε RPC (αλλιώς δε στέλνουμε κάθε frame)
        if (currentTeam != lastTeam && photonView != null)
        {
            photonView.RPC("UpdateZoneMaterial", RpcTarget.AllBuffered, currentTeam);
    ...
    private void UpdateZoneMaterial(string team)
    {
        ...
        if (rawImageToChangeColor != null)
        {
            if (team == "blue")
                ...
                rawImageToChangeColor.color = new Color(1f, 0f, 0f, 0.8f);
            else
                rawImageToChangeColor.color = originalRawImageColor;
        }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (setup == null)
            setup = other.GetComponentInParent<PlayerSetup>();
        if (setup == null) return;

        int actorId = setup.photonView.Owner.ActorNumber;

        if (setup.myTeam == PlayerSetup.Team.Blue)
    ...
    private void OnTriggerExit(Collider other)
{
    ...
    // <---  ADD THIS PART
    // Αν δεν υπάρχει ΚΑΝΕΝΑΣ πια στη ζώνη => γίνεται neutral παντού και μηδενίζουμε το lastTeam
    if (bluePlayersInside.Count == 0 && redPlayersInside.Count == 0)
    {
        lastTeam = "neutral";

        if (photonView != null)
            photonView.RPC("UpdateZoneMaterial", Photon.Pun.RpcTarget.AllBuffered, "neutral");
